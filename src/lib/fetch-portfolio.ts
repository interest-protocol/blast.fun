...
import type { PortfolioBalanceItem, PortfolioResponse } from "@/types/portfolio"
...
export async function fetchPortfolio(address: string): Promise<PortfolioResponse> {
	try {
		...
		const allBalances: PortfolioBalanceItem[] =
			nexaPortfolio.balances?.map((item: any) => ({
		...
		const xpumpBalances = allBalances.filter((item) => item.coinMetadata?.platform === "xpump")
