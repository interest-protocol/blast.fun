...
function mapNoodlesTradeToMarketTrade(
...
export async function fetchMarketTrades({
	...
	if (poolAddress) {
		const { trades: rawTrades } = await fetchNoodlesPoolTradeEvents(
			poolAddress,
			pageSize
		)
	...
	const { trades: rawTrades } = await fetchNoodlesPoolTradeEventsByProtocols(
		pageSize,
		undefined,
		coinType
	)
