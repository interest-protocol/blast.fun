...
export class VestingApi {
	static async getVestingsByCoinType(coinType: string, params: VestingApiParams = {}): Promise<VestingByCoinResponse> {
	...
	static async getAllVestingsByCoinType(coinType: string): Promise<VestingByCoinResponse> {
		const limit = 50;
		let offset = 0;
	...
	static async getVestingsByUser(address: string, params: VestingApiParams = {}): Promise<VestingByUserResponse> {
		const { limit = 20, offset = 0 } = params;
