...
class CoinMetadataApi {
	...
	async getCoinMetadata(coinType: string, options: CoinMetadataApiOptions = {}): Promise<CoinMetadata | null> {
		try {
			const response = await fetch(`${this.baseUrl}/fetch-coins?coinTypes=${encodeURIComponent(coinType)}`, {
				headers: {
	...
	async searchCoins(search: string, options: CoinMetadataApiOptions = {}): Promise<CoinMetadata[]> {
		try {
			const response = await fetch(`${this.baseUrl}/fetch-coins?search=${encodeURIComponent(search)}`, {
				headers: {
					Accept: "application/json",
				},
			});
	...
	async getBatchCoinMetadata(coinTypes: string[], options: CoinMetadataApiOptions = {}): Promise<CoinMetadata[]> {
		try {
