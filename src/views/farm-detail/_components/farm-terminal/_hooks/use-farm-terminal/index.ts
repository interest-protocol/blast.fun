...
import { POW_9 } from "@/views/farms/farms.const";

const useFarmTerminal = ({ farm, account, metadata, onOperationSuccess }: FarmTerminalProps) => {
	...
	useEffect(() => {
		const fetchRewardMetadata = async () => {
			...
			try {
				...
				if (!res.ok) return;
				const md = (await res.json()) as TokenMetadata;
