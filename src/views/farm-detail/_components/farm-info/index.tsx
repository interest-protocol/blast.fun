...
import { POW_9, SECONDS_IN_YEAR } from "@/views/farms/farms.const";

const FarmInfo: FC<FarmInfoProps> = ({ farm, account, metadata, onOperationSuccess }) => {
	const rewardCoinType = farm.rewardTypes[0] || "";

	const { stakeTokenPrice, rewardTokenPrice } = useFarmPrices(farm.stakeCoinType, rewardCoinType);

	const { rewardMetadata } = useRewardMetadata(rewardCoinType);

	const { pendingRewards, refreshCountdown } = usePendingRewards(account?.objectId, rewardCoinType);
