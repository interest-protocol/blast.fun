...
import type { AirdropRecipient } from "../airdrop-tools/airdrop-tools.types";
...
export function useAirdrop({ address, selectedCoin, recipients, csvInput }: UseAirdropParams) {
	...
	const fundDelegator = async (
		...
		tx.transferObjects(coinInput ? [coinInput, gasInput] : [gasInput], delegatorAddr);
	...
	const refundDelegator = async () => {
		...
		try {
			...
			const nonSuiCoins = allCoins.data.filter((coin) => coin.coinType !== SUI_TYPE_ARG && BigInt(coin.balance) > 0n);
	...
	const recoverGas = async (delegatorKeypair: Ed25519Keypair, delegatorAddr: string, recipient: string) => {
