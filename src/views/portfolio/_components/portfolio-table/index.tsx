...
const PortfolioTable: FC<PortfolioTableProps> = ({ portfolio, hideSmallBalance, onHideSmallBalanceChange }) => {
	...
	const filteredBalances = useMemo(
		() => getFilteredBalances(portfolio.balances, hideSmallBalance),
		[portfolio, hideSmallBalance]
	);
	const sortedBalances = useMemo(
		() => getSortedBalances(filteredBalances, sortField, sortOrder),
		[filteredBalances, sortField, sortOrder]
	);
				...
				<table className="w-full">
					...
					<tbody className="divide-y divide-border">
						{sortedBalances.map((item) => (
							<PortfolioTableRow key={item.coinType} item={item} />
						))}
					</tbody>
				</table>
			</div>
