			...
			<td className="px-3 md:px-6 py-4 text-right">
				<div className="flex flex-col items-end">
					<p className="font-mono font-semibold text-sm md:text-base text-foreground/80">
						{formatTokenAmount(item.balance, 0)}
					</p>
					<p className="font-mono text-xs text-muted-foreground/60">${formatNumber(item.value)}</p>
				</div>
			</td>
			...
			<td className="px-3 md:px-6 py-4 text-right">
				<div className="flex flex-col items-end gap-1">
					<div className="flex items-center gap-1">
						...
						<p className={cn("font-mono font-semibold", isProfitable ? "text-green-500" : "text-destructive")}>
							${formatNumber(Math.abs(item.unrealizedPnl))}
						</p>
					</div>
					<p className={cn("font-mono text-xs", isProfitable ? "text-green-500" : "text-destructive")}>
						{isProfitable ? "+" : ""}
