...
export function TwitterRelationsProvider({ children, pool }: TwitterRelationsProviderProps) {
	...
	const {
		...
		enabled: !!pool.pool?.poolId && pool.pool?.isProtected && protectionSettings?.revealTraderIdentity === true,
	...
	return <TwitterRelationsContext.Provider value={value}>{children}</TwitterRelationsContext.Provider>;
}
