...
export function TradeTerminal({ pool, referral }: TradeTerminalProps) {
	...
	const { slippage, quickBuyAmounts, quickSellPercentages, setQuickBuyAmounts, setQuickSellPercentages } =
		usePresetStore();
	...
	const { price: suiPrice = 0 } = usePrice({ coinType: SUI_TYPE_ARG });
	...
	const tokenBalanceDisplay = tokenBalanceRaw ? Number(tokenBalanceRaw) / Math.pow(10, decimals) : 0;

	// portfolio balance (already in display units)
	const portfolioBalanceDisplay = actualBalance !== "0" ? Number(actualBalance) : 0;

	// for UI we prefer the on-chain balance; fall back to portfolio if needed
	const balanceInDisplayUnit = tokenBalanceDisplay || portfolioBalanceDisplay || 0;
	...
	useEffect(() => {
		const calculateMaxBuyable = async () => {
			...
			try {
				...
				const totalSupplyInSmallestUnit = TOTAL_POOL_SUPPLY * BigInt(10 ** decimals);
				...
				const remainingTokensToBuy =
					maxTokenAmount > currentHoldingsBigInt ? maxTokenAmount - currentHoldingsBigInt : 0n;
				...
				let low = 0n;
				let high = BigInt(suiBalanceInDisplayUnit * Number(MIST_PER_SUI));
	...
	const handleQuickAmount = (value: number) => {
		if (tradeType === "buy") {
			...
			if (percentage === 100) {
				setAmount(new BigNumber(balanceInDisplayUnit).toFixed(decimals, BigNumber.ROUND_DOWN));
			} else {
				try {
					const balanceBN = new BigNumber(balanceInDisplayUnit);
					const percentageBN = new BigNumber(percentage).dividedBy(100);
					const tokenAmount = balanceBN.multipliedBy(percentageBN).toFixed(decimals, BigNumber.ROUND_DOWN);
	...
	const handleTrade = async () => {
		...
		if (tradeType === "buy") {
			...
			await Promise.all([refetchPortfolio(), refetchTokenBalance(), refetchSuiBalance()]);
			...
			await burn(amount);
			await Promise.all([refetchPortfolio(), refetchTokenBalance()]);
		...
		return (
			<div className="p-4 border-b border-border">
				<div className="text-center space-y-2">
					...
					<p className="font-mono text-xs text-muted-foreground">Connect wallet to trade</p>
				</div>
			</div>
		);
	}
	...
	return (
		<div className="relative border-b border-border">
			...
			<div className="p-3 space-y-3">
				{/* Buy/Sell/Burn Tabs */}
				<div className={cn("grid gap-1 p-1 bg-muted/30 rounded-lg", hasBalance ? "grid-cols-3" : "grid-cols-2")}>
					<button
						...
						<div className="flex items-center gap-2">
							...
							<button
								...
								disabled={
									...
									(tradeType === "buy" && isCalculatingMax)
								}
							>
						...
						<div className="flex items-center gap-2">
							<input
								...
								onChange={(e) => {
									...
									if (raw === "" || /^\d*\.?\d*$/.test(raw)) {
										const maxDecimals = tradeType === "buy" ? 9 : decimals;
				...
				{!pool.pool?.migrated && protectionSettings?.requireTwitter && !isTwitterLoggedIn && tradeType !== "burn" ? (
					<Button variant="outline" className="w-full h-10 font-mono text-xs uppercase" onClick={twitterLogin}>
