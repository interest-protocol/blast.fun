...
export function ReferralShare({ pool }: ReferralShareProps) {
	...
	const tokenPageUrl = refCode ? `${origin}/token/${pool.coinType}?ref=${refCode}` : `${origin}/token/${pool.coinType}`;
		...
		return (
			<div className="relative border-b border-border">
				<div className="p-3 space-y-3">
					...
					{error && <p className="font-mono font-medium text-xs uppercase text-destructive">{error}</p>}
				</div>
			</div>
		);
	}
