...
const TokenItem: FC<TokenItemProps> = ({ token }) => {
	return (
		<Link href={`/token/${token.coinType}`} className="block p-4 hover:bg-muted/5 transition-colors" target="_blank">
			<div className="flex items-center gap-4">
				{token.iconUrl ? (
					...
					<div className="w-12 h-12 bg-muted rounded-full flex items-center justify-center">
						<span className="text-xs font-mono text-muted-foreground">{token.symbol?.slice(0, 2) ?? "??"}</span>
					</div>
				)}
				...
				<div className="flex-1">
					...
					{token.description && (
						<p className="text-xs text-muted-foreground mt-1 line-clamp-1">{token.description}</p>
					)}
