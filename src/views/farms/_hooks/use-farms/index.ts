...
export const useFarms = () => {
	...
	const fetchFarmsData = async () => {
		...
		try {
			...
			const farmsData = await Promise.all(farmIds.map((farmId) => farmsSdk.getFarm(farmId)));
			...
			if (address && isConnected) {
				...
				farmsWithAccountsData = farmsData.map((farm) => {
					...
					const primaryAccount = farmAccounts.sort((a, b) => Number(b.stakeBalance - a.stakeBalance))[0];
