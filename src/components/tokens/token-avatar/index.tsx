...
const TokenAvatar: FC<TokenAvatarProps> = ({
	...
	return (
		<HoverCard openDelay={200} closeDelay={100}>
			...
			<HoverCardContent className="p-1.5 bg-background/95 backdrop-blur-sm border-2 w-auto" side="top" sideOffset={5}>
				<div className="relative">
					...
					{symbol && (
						<div className="absolute bottom-1 left-1 right-1 bg-black/60 backdrop-blur-sm rounded px-2 py-1">
							<p className="text-white text-xs font-mono font-bold text-center truncate">{symbol}</p>
						</div>
					)}
				</div>
			</HoverCardContent>
