...
export const TokenListLayout = memo(function TokenListLayout({
	...
	return (
		<div className={cn("flex min-h-0 flex-col overflow-hidden rounded-xl border border-border/50", className)}>
			<div
				className={cn(
					"flex flex-shrink-0 items-center justify-between border-border/50 border-b px-4 py-3",
					headerClassName
				)}
			>
				<h2 className={cn("font-bold font-mono text-xs uppercase tracking-wider", textGlowStyles[glowColor])}>
					{title}
				</h2>
