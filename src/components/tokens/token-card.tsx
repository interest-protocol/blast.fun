...
export const TokenCard = memo(function TokenCard({ pool: tokenData }: TokenCardProps) {
	...
	return (
		<Link href={`/token/${token.coinType}`} className="cursor-default">
			<div className="group relative overflow-hidden border-border/40 border-b transition-all duration-300 hover:bg-accent/15">
				{/* Content */}
				<div className="relative p-3 sm:p-2">
					<div className="flex gap-3 sm:gap-2.5">
						<div className="flex-shrink-0">
							<div className="flex h-[48px] w-[48px] flex-col items-center justify-start sm:h-[56px] sm:w-[56px]">
								{/* avatar with conic progress */}
								<div
									className={`relative flex-shrink-0 overflow-hidden rounded-md ${
										isGraduated
											? "h-[48px] w-[48px] sm:h-[56px] sm:w-[56px]"
											: "h-[38px] w-[48px] sm:h-[44px] sm:w-[56px]"
									}`}
								>
									...
									<div className="mt-1 flex w-full items-center justify-center">
										<p
											className={`font-mono font-semibold text-[10px] ${
												bondingProgress >= 30 ? "text-pink-500/80" : "text-blue-500/80"
											}`}
										>
						...
						<div className="min-w-0 flex-1 space-y-1">
							<div className="flex items-center gap-2">
								<h3 className="truncate font-bold font-mono text-foreground/90 text-xs uppercase tracking-wider sm:text-sm">
