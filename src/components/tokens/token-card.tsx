...
export const TokenCard = memo(function TokenCard({ pool: token, hasRecentTrade = false, column }: TokenCardProps) {
    ...
    const bondingProgress = token.bondingCurveProgress
        ?? (token.bondingProgress ? token.bondingProgress * 100 : 0)

    const twitter = token.twitter ?? token.socialMedia?.x
    const telegram = token.telegram ?? token.socialMedia?.telegram
    const website = token.website ?? token.socialMedia?.website

    const volume = token.volume24h ?? ((token.buyVolume ?? 0) + (token.sellVolume ?? 0))
    ...
    return (
        <Link href={`/token/${token.coinType}`} className="cursor-default">
            <div className={`group relative overflow-hidden border-border/40 border-b transition-all duration-300 hover:bg-accent/15 ${hasRecentTrade ? "animate-shake" : ""}`}>
                                ...
                                <div className="relative h-[48px] w-[48px] flex-shrink-0 overflow-hidden rounded-md sm:h-[56px] sm:w-[56px]">
                                    <div className="relative h-full w-full">
                                        <div className={`absolute inset-0 rounded-md ${
                                            bondingProgress >= 100
                                                ? "bg-gradient-to-br from-yellow-600/30 to-amber-600/30"
                                                : bondingProgress >= 30
                            ...
                            <div className="flex flex-wrap items-center gap-2 font-mono text-xs sm:gap-3">
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <div className="flex items-center gap-1">
                                                <span className="text-[9px] text-muted-foreground/60 uppercase tracking-wider sm:text-[10px]">MC</span>
                                                <span className="font-semibold text-[11px] text-green-500/90 sm:text-xs">
                                                    ${formatNumberWithSuffix(token.marketCap)}
                                                </span>
                                            </div>
                                        </TooltipTrigger>
                                        <TooltipContent><p className="font-mono text-xs uppercase">MARKET CAP</p></TooltipContent>
                                    </Tooltip>
                                )}
                                ...
                                {volume > 0 && (
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <div className="flex items-center gap-1">
                                                <span className="text-[9px] text-muted-foreground/60 uppercase tracking-wider sm:text-[10px]">VOL</span>
                                                <span className="font-semibold text-[11px] text-purple-500/90 sm:text-xs">
                                                    ${formatNumberWithSuffix(volume)}
                                                </span>
                                            </div>
                                        </TooltipTrigger>
                                        <TooltipContent><p className="font-mono text-xs uppercase">24H VOLUME</p></TooltipContent>
                                    </Tooltip>
                                )}
                                        ...
                                        <TooltipTrigger asChild>
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="font-semibold text-[11px] text-foreground/70 sm:text-xs">
                                                    {formatNumberWithSuffix(holders)}
                                                </span>
                                            </div>
                                        </TooltipTrigger>
                                        <TooltipContent><p className="font-mono text-xs uppercase">HOLDERS</p></TooltipContent>
                                    </Tooltip>
                                )}
                            </div>
                                            ...
                                            {socialLinks.map((link, index) => {
                                                ...
                                                return (
                                                    <Tooltip key={index}>
                                                        ...
                                                        <TooltipContent><p className="font-mono text-xs uppercase">{link.tooltip}</p></TooltipContent>
                                                    </Tooltip>
                                                )
                                            })}
                                        </div>
