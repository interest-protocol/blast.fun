...
export const GraduatedComplete = memo(function GraduatedComplete({ pollInterval = 30000 }: GraduatedCompleteProps) {
	...
	const renderContent = useCallback(() => {
		if (error) {
			return (
				<div className="p-8 text-center">
					...
					<p className="font-mono text-xs uppercase text-destructive">ERROR::LOADING::GRADUATED</p>
				</div>
			);
		}
		...
		return filteredAndSortedTokens.map((coin) => {
			...
			return (
				<TokenCard key={coin.coinType} pool={pool} hasRecentTrade={isAnimating(coin.coinType)} column="graduated" />
			);
		});
