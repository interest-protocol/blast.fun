...
const WalletAccountItem: FC<WalletAccountItemProps> = ({ account, isActive, onSelect }) => {
	...
	return (
		<Button
			variant="ghost"
			onClick={onSelect}
			className={cn("w-full justify-start h-auto py-2", isActive && "bg-background/60")}
		>
			<div className="flex items-center justify-between w-full">
				<div className="flex items-center gap-2">
					<Wallet className="size-4 text-muted-foreground" />
					<span className="text-sm font-normal">{displayAddress}</span>
				</div>

				<div className="flex items-center gap-1">
					<Button type="button" variant="ghost" size="icon" onClick={handleCopy} className="size-5">
						{copied ? (
					...
					{isActive && <Check className="size-4 text-green-400" />}
				</div>
			</div>
		</Button>
	);
