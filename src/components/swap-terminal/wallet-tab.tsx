...
export const WalletTab: FC<WalletTabProps> = ({ searchQuery, onSelectToken, disabledCoinTypes = [] }) => {
	...
	if (!isConnected) {
		return (
			<div className="flex flex-col w-full items-center justify-center p-8 text-center min-h-[200px]">
				...
				<p className="text-sm font-mono uppercase tracking-wider text-foreground mb-2">WALLET::NOT_CONNECTED</p>
				<p className="text-xs text-muted-foreground mb-6 max-w-sm">Connect your wallet to view your tokens</p>
				<Button onClick={() => setIsConnectDialogOpen(true)} className="font-mono uppercase text-xs">
					Connect Wallet
				</Button>
			</div>
		);
