...
export const TokensTab: FC<TokensTabProps> = ({ searchQuery, onSelectToken, fromToken, toToken }) => {
	const disabledCoinTypes = [fromToken?.coinType, toToken?.coinType]
		.filter((coinType): coinType is string => coinType !== null && coinType !== undefined)
		.map((coinType) => normalizeStructTag(coinType));
	const [expandedSection, setExpandedSection] = useState<TokenSection | null>("verified");
	...
	return (
		<div className="flex flex-col flex-1 overflow-y-auto">
			<Collapsible
				...
				className={cn("flex flex-col overflow-hidden", expandedSection === "verified" && "flex-1")}
			>
				<CollapsibleTrigger
					className={cn(
				...
				>
					<span className="text-xs font-mono uppercase tracking-wider text-foreground">VERIFIED</span>
			...
			<Collapsible
				...
				className={cn("flex flex-col overflow-hidden", expandedSection === "wallet" && "flex-1")}
			>
				<CollapsibleTrigger
					className={cn(
						"w-full flex items-center justify-between px-4 py-3 border-b border-border/50 hover:bg-muted/10 transition-colors",
				...
				>
					<span className="text-xs font-mono uppercase tracking-wider text-foreground">WALLET</span>
