...
export const useTokenBalances = (fromToken: TokenOption | null, toToken: TokenOption | null) => {
	const { balance: fromBalance, refetch: refetchFromBalance } = useTokenBalance(fromToken?.coinType);
	const { balance: toBalance, refetch: refetchToBalance } = useTokenBalance(toToken?.coinType);
