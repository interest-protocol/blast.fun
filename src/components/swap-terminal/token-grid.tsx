...
export const TokenGrid: FC<TokenGridProps> = ({ tokens, isLoading, searchQuery, onSelectToken, disabledCoinTypes = [] }) => {
	...
	if (tokens.length === 0)
		return (
			<div className="flex w-full justify-center items-center text-center text-muted-foreground text-sm">
				{searchQuery.length >= MIN_SEARCH_LENGTH ? "No tokens found." : "No tokens available."}
			</div>
		);
	...
	return (
		<div className="p-4 items-start grid overflow-y-auto grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
			{tokens.map((token) => {
				const normalizedTokenCoinType = normalizeStructTag(token.coinType);
				const isDisabled = disabledCoinTypes.includes(normalizedTokenCoinType);
				return (
					<button
						key={token.coinType}
						onClick={() => {
