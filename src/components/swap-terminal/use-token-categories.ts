...
export const useTokenCategories = (activeCategory: TokenCategory, enabled: boolean = true) => {
	const latestTokensQuery = useLatestTokens(undefined, {
		enabled: enabled && activeCategory === "newly-created",
		refetchInterval: enabled && activeCategory === "newly-created" ? NEWLY_CREATED_REFETCH_INTERVAL : undefined,
	});

	const aboutToBondQuery = useAboutToBondTokens(undefined, {
		enabled: enabled && activeCategory === "near-graduated",
		refetchInterval: enabled && activeCategory === "near-graduated" ? NEAR_GRADUATED_REFETCH_INTERVAL : undefined,
	});

	const bondedTokensQuery = useBondedTokens(undefined, {
		enabled: enabled && activeCategory === "graduated",
		refetchInterval: enabled && activeCategory === "graduated" ? GRADUATED_REFETCH_INTERVAL : undefined,
	});
