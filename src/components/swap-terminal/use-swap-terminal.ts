...
import { DEFAULT_SLIPPAGE, DEFAULT_SUI_TOKEN, SUI_RESERVE_AMOUNT } from "./swap-terminal.data";
...
export const useSwapTerminal = () => {
	...
	const [slippage, setSlippage] = useLocalStorage("swap-terminal-slippage", DEFAULT_SLIPPAGE);
	...
	const { fromBalanceDisplay, toBalanceDisplay, refreshBalances } = useTokenBalances(fromToken, toToken);
	...
	const isValidAmount = parseFloat(fromAmount) <= fromBalanceDisplay && parseFloat(fromAmount) > 0;
