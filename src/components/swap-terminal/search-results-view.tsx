...
import { useLatestTokens, useAboutToBondTokens, useBondedTokens } from "@/hooks/use-tokens";
...
export const SearchResultsView: FC<SearchResultsViewProps> = ({
	...
	const [expandedSections, setExpandedSections] = useState<Set<SectionKey>>(new Set(["global", "verified"]));

	const { tokens: verifiedTokens, isLoading: isLoadingVerified } = useVerifiedTokens();
	const { tokens: walletTokens, isLoading: isLoadingWallet } = useWalletTokens();
	const { data: newlyCreatedData, isLoading: isLoadingNewlyCreated } = useLatestTokens();
	const { data: nearGraduatedData, isLoading: isLoadingNearGraduated } = useAboutToBondTokens();
	const { data: graduatedData, isLoading: isLoadingGraduated } = useBondedTokens();

	const filteredGlobalResults = useMemo(() => globalSearchResults, [globalSearchResults]);

	const filteredVerifiedTokens = useMemo(() => filterTokens(verifiedTokens, searchQuery), [verifiedTokens, searchQuery]);

	const filteredWalletTokens = useMemo(() => filterTokens(walletTokens, searchQuery), [walletTokens, searchQuery]);
			...
			{visibleSections.map((section) => {
				...
				return (
					<Collapsible
						...
						className={cn("flex flex-col overflow-hidden", isExpanded && "flex-1")}
					>
						<CollapsibleTrigger
							className={cn(
						...
						>
							<span className="text-xs font-mono uppercase tracking-wider text-foreground">
								{section.label}
								{hasResults && <span className="ml-2 text-muted-foreground">({section.tokens.length})</span>}
							</span>
							{isExpanded ? (
