...
export const useSwapQuote = ({ fromToken, toToken, fromAmount, slippage }: UseSwapQuoteProps) => {
	...
	useEffect(() => {
		const fetchQuote = async () => {
			...
			try {
				...
				const amountIn = BigInt(Math.floor(parseFloat(fromAmount) * Math.pow(10, decimals)));
				...
				const amountOut = Number(quote.amountOut) / Math.pow(10, outDecimals);
