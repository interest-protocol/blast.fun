...
export function TokenInfo({ pool, realtimePrice, realtimeMarketCap }: TokenInfoProps) {
	...
	const priceChange5m = priceData?.price5MinsAgo && priceData?.coinPrice
		? ((priceData.coinPrice - priceData.price5MinsAgo) / priceData.price5MinsAgo) * 100
		: null

	const priceChange1h = priceData?.price1HrAgo && priceData?.coinPrice
		? ((priceData.coinPrice - priceData.price1HrAgo) / priceData.price1HrAgo) * 100
		: null

	const priceChange4h = priceData?.price4HrAgo && priceData?.coinPrice
		? ((priceData.coinPrice - priceData.price4HrAgo) / priceData.price4HrAgo) * 100
		: null

	const priceChange24h = priceData?.price1DayAgo && priceData?.coinPrice
		? ((priceData.coinPrice - priceData.price1DayAgo) / priceData.price1DayAgo) * 100
		: null
	...
	const isCreator = isConnected && address && pool.creatorAddress && address === pool.creatorAddress
		...
		<>
			<div className="bg-background">
				<div>
					...
					{(pool.isProtected || protectionSettings || (pool.burnTax && pool.burnTax > 0)) && (
