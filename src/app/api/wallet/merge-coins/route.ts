import { GasStationError, suiSponsorship } from "@3mate/gas-station-sdk"
...
export async function POST(req: NextRequest) {
	try {
		...
		if (!coins || !Array.isArray(coins) || coins.length === 0) {
			return NextResponse.json({ error: "Coins array is required" }, { status: 400 })
		}

		if (!coinType || !walletAddress) {
			return NextResponse.json({ error: "coinType and walletAddress are required" }, { status: 400 })
		}

		if (!GAS_STATION_API_KEY) {
			return NextResponse.json({ error: "Gas station not configured" }, { status: 500 })
		}
		...
		try {
			...
			if (error instanceof GasStationError) {
				return NextResponse.json({ error: `Gas station sponsorship failed: ${error.message}` }, { status: 500 })
			}
			throw error
		}
