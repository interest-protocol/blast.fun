...
export async function GET(_request: Request, { params }: { params: Promise<{ address: string; coin_type: string }> }) {
	...
	try {
		...
		const coin = portfolio?.data?.find((c) => c.coin_type === decodedCoinType);
