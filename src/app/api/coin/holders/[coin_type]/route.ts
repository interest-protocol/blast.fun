...
export async function GET(
	...
	try {
		...
		try {
			...
			const trueBurnedSupply = String((1000000000000000000n - BigInt(totalSupply)) / 10n ** 9n)
			...
			if (parseFloat(trueBurnedSupply) > 0) {
				holders.push({
					...
					percentage: (Number(trueBurnedSupply) / 1_000_000_000).toString(),
					name: "",
					image: "",
					website: ""
				});
			...
			try {
				...
				if (!holdersResponse.success || !holdersResponse.data) {
					console.error(`❌ Both APIs failed. BlockVision error: ${holdersResponse.error}`)
					return NextResponse.json(
						{ error: holdersResponse.error || "Failed to fetch holders from both APIs" },
						{ status: 500 }
					)
				}
				
				holders = holdersResponse.data
				console.log(`✅ Got ${holders.length} holders from BlockVision API (fallback)`)
			} catch (blockVisionError) {
				console.error(`❌ Both APIs failed. BlockVision error: ${blockVisionError}`)
				return NextResponse.json(
		...
		console.log(`✅ Fetched and cached ${holders.length} holders for ${coinType} from ${isFromNexa ? 'Nexa' : 'BlockVision'}`)
