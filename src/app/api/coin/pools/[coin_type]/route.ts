...
export async function GET(request: Request, { params }: { params: Promise<{ coin_type: string }> }) {
	try {
		...
		if (!poolsResponse.success || !poolsResponse.data) {
			...
			return NextResponse.json({ error: poolsResponse.error || "Failed to fetch DEX pools" }, { status: 500 })
		}
		...
		return NextResponse.json({ error: "Failed to fetch coin pools" }, { status: 500 })
	}
}
